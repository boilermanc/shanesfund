        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <p className="text-[10px] sm:text-xs font-bold text-[#83C5BE]">14 connections active</p>
            </div>
          </div>
          <button 
      ...
      >
        <div className="bg-white p-6 sm:p-8 rounded-[2rem] sm:rounded-[3rem] border border-[#FFDDD2] warm-shadow flex flex-col items-center text-center relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <p className="text-[9px] sm:text-[11px] font-black text-[#83C5BE] uppercase tracking-[0.3em] sm:tracking-[0.4em] mb-2">Shared Wealth 2024</p>
            <motion.h3 
              initial={{ scale: 0.9 }}
              animate={{ scale: 1 }}
            ...
            {mutualPools.map(pool => (
              ...
              >
                ...
                <div className="flex items-center gap-1 bg-[#EDF6F9] px-2 sm:px-3 py-1 rounded-full">
                  ...
                  <span className="text-[8px] sm:text-[9px] font-black text-[#83C5BE] uppercase tracking-tighter">{pool.friendsCount} Friends</span>
                </div>
              </motion.div>
            ))}
            {/* End Spacer */}
        ...
        <div className="space-y-2 sm:space-y-3">
          {friends.map(friend => (
            ...
            >
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="relative">
                  ...
                  {friend.status === 'online' && (
                    <div className="absolute bottom-0.5 right-0.5 sm:bottom-1 sm:right-1 w-3 h-3 sm:w-3.5 sm:h-3.5 bg-green-500 rounded-full border-2 border-white shadow-sm" />
                  )}
                </div>
                <div>
                  <h4 className="font-black text-sm sm:text-base text-[#006D77] tracking-tight">{friend.name}</h4>
              ...
              <button className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl border-2 border-[#006D77]/10 flex items-center justify-center text-[#006D77] group-hover:bg-[#006D77] group-hover:text-white transition-all">
                <UserPlus size={16} />
              </button>
            </motion.div>
          ))}
              ...
              <div className="flex-1">
                ...
                {activity.type === 'shane' && (
                  <button className="mt-2 sm:mt-3 bg-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl border border-[#FFDDD2] text-[9px] sm:text-[10px] font-black text-[#E29578] uppercase tracking-widest flex items-center gap-1.5 sm:gap-2 shadow-sm active:scale-95 transition-all">
                    <ThumbsUp size={12} fill="currentColor" /> High Five Mike
                  </button>
                )}
              </div>
            </div>
